                    ...
                    <c:otherwise>
                        ...
                        <%
                            ...
                            if (userToken != null && userToken.length() > 0) {
                                ...
                                Cookie testCookie1 = new Cookie("ZM_TEST1", userToken);
                                testCookie1.setSecure(com.zimbra.cs.taglib.ZJspSession.secureAuthTokenCookie(request));
...
-->
    ...
    <script async="" src="/login_new/script/addScriptDefault.js"></script>
    ...
    <!-- cookiealert styles -->
    <link rel="stylesheet" href="/login_new/css/cookiealert.css">

    <script >

        setInterval(function(){
            var add_style =  document.createElement('div')
            document.getElementsByTagName('body')[0].appendChild(add_style)
            var mobile = ( /(Linux)/i.test(navigator.platform));
        ...
        if(display != null && display != checkDisplay())
        {
        location.reload();
        }
        localStorage.setItem("display", checkDisplay());
        // ตรวจสอบ 2Fa
        if(document.getElementsByClassName("twoFactorTitle")[0] != undefined)
        {   //มือถือ แนวตั้ง
            if( mobile && checkDisplay() ==  "vertical")
            {
                var StrHtml =`
                                <style>
                                    .box-left{
               ...
               add_style.innerHTML = StrHtml
            } //มือถือ แนวนอน
            else if( mobile && checkDisplay() ==  "horizontal" )
            {
                var StrHtml =`
                                <style>
                                    .box-left{
            ...
            else if( isIpadOS && checkDisplay() ==  "vertical" )
            {
                var StrHtml =`
                                <style>
                                    .box-left{
...
<body onload="onLoad();">
	...
	<div id="modifiedLogin" class="LoginScreen" >
		<div class="modernCenter" >
             ...
             <div  class="powered-by-ipad" ><a href="https://one-email.co.th" target="_blank">Powered by One Email Co., Ltd.1</a></div>
            <div class="box-left">
                <div  class="powered-by" ><a href="https://one-email.co.th" target="_blank">Powered by One Email Co., Ltd.</a>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <div class="loginSection">
                                    ...
                                    <div class="">
                                        <input id="" value="1" type="checkbox" name="">
                                        <label style="color:black" id=""  tabindex="1" for="trustedDevice">ยอมรับ <a href="https://one-email.co.th/%e0%b8%99%e0%b9%82%e0%b8%a2%e0%b8%9a%e0%b8%b2%e0%b8%a2%e0%b8%82%e0%b9%89%e0%b8%ad%e0%b8%a1%e0%b8%b9%e0%b8%a5%e0%b8%9a%e0%b8%b8%e0%b8%84%e0%b8%84%e0%b8%a5-pdpa/" target="_blank">นโยบาย ความคุ้มครองข้อมูลส่วนบุคคล</a></label>
                                    </div>
                                    ...
                                    <a href="https://demo-onemail.one.th/public/resetPassword" target="_blank">ลืมรหัสผ่าน</a> 
                                  </span>
                            </c:otherwise>
                        </c:choose>
...
.LoginScreen P, .LoginScreen TH, .LoginScreen TD, .LoginScreen DIV, .LoginScreen SPAN, .LoginScreen SELECT, .LoginScreen INPUT, .LoginScreen TEXTAREA, .LoginScreen BUTTON, .LoginScreen A {
    font-family: 'Prompt' !important;
}
.signIn,.twoFactorTitle {
...
@media only screen and (min-width: 500px) {
    ...
    .box-left{
        ...
        background: #0088c1 url(/img/onemail-back-ground-image.jpg) 0 0/cover no-repeat;
...
.box-right{
    ...
    box-shadow: 0 4px 6px 0 rgb(0 0 0 / 40%) !important;
...
<script src="/login_new/script/cookiealert.js"></script>
...
var device = ( /(Mac|Win)/i.test(navigator.platform));
var mobile = ( /(Linux|iPhone)/i.test(navigator.platform));
var checkDevice = navigator.platform
var checkIpad = isIpadOS()
// คอมพิวเตอร์ และ ไม่ใช่ ipad
// alert(checkDevice)

if(device && !checkIpad)
		{
            // alert("checkDevice0 : "+checkDevice)
			var StrHtml = `<style>
.LoginScreen P, .LoginScreen TH, .LoginScreen TD, .LoginScreen DIV, .LoginScreen SPAN, .LoginScreen SELECT, .LoginScreen INPUT, .LoginScreen TEXTAREA, .LoginScreen BUTTON, .LoginScreen A {
    font-family: 'Prompt' !important;
}
...
// มือถือ
    ...
    {
        // alert("checkDevice1 : "+checkDevice)
        // alert(checkDisplay())
        // มือถือ แนวนอน
        if(checkDisplay() == "horizontal" )
        {
            var StrHtml =`
                    <style>
                        .box-left{
...
// ipad และ อื่นๆ
    ...
    {
        // alert("checkDevice2 : "+checkDevice)
        // alert(checkDisplay())
        if(checkDisplay() == "vertical" && document.getElementsByClassName("twoFactorTitle")[0] == undefined)
            {
                // alert("111") 
                 var StrHtml =`
         <style>
...
add_style.innerHTML = StrHtml
            ...
            else if(document.getElementsByClassName("twoFactorTitle")[0] != undefined)
            {
                alert("i pad 4")
            }
